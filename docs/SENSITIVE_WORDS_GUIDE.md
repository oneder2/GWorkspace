...
系统支持通过管理 API 动态添加敏感词。该操作会自动更新磁盘文件并**刷新内存中的正则缓存**，无需重启服务器即可生效。

## 性能优化：正则缓存机制

为了确保在大规模文本过滤时的性能，后端实现了以下机制：
1. **预编译缓存**：在启动和更新时，所有敏感词会被预编译为正则表达式。
2. **中英差异匹配**：中文采用全文匹配，英文采用单词边界 (`\b`) 匹配，兼顾准确性与性能。
3. **低延迟**：即使词库包含数千个词条，过滤操作仍能保持在毫秒级延迟。
