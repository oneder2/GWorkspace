...
GWorkspace 整个站点就是工作空间，不再把某一个路由包装成第二层“工作台”。普通路由是同一工作空间中的功能区域：首页是桌面，`/workspace` 是工具箱，博客是写作区，作品集是档案区。`WorkShelf` 常驻主壳体底部，负责显示当前区域与最近活动，让未完成的工作在切换路由后仍然可见。

`/workspace` 只承担工具选择和工具运行，不承载全站状态面板。其总览应保持紧凑、按用途分组，外部资源使用次级抽屉呈现；工具组件内部的业务逻辑保持独立。
